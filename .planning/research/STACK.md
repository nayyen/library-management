...
**Domain:** University library management system (catalog browsing, request/approval workflows, role-based auth, email notifications)
**Researched:** 2026-06-08
**Confidence:** HIGH (core stack verified via Context7/PyPI/npm registries; ecosystem patterns MEDIUM via multiple cross-checked sources)

This research takes the user's stack choice (FastAPI + React + PostgreSQL + Docker) as fixed and focuses on **how to implement it well** for this specific domain — not on alternatives to the stack itself.
...
uv add fastapi "uvicorn[standard]" sqlalchemy alembic asyncpg "psycopg[binary]" \
       pydantic-settings "pwdlib[argon2]" pyjwt python-multipart \
       fastapi-mail jinja2 apscheduler

uv add --dev pytest pytest-asyncio httpx ruff mypy pre-commit factory-boy
...
npm install -D tailwindcss @tailwindcss/vite vitest @testing-library/react \
               @testing-library/jest-dom openapi-typescript openapi-fetch

# shadcn/ui is added via its CLI (copies component source into your repo):
npx shadcn@latest init
npx shadcn@latest add button table dialog form input dropdown-menu badge
```
...
**For the auth/role model in this project (student vs. librarian, no SSO):**
...
**For the catalog + loan workflow (the domain's core complexity):**
...
**For "thousands of books, hundreds/thousands of students" scale:**
...
---
*Stack research for: University library management system (FastAPI + React + PostgreSQL + Docker)*
*Researched: 2026-06-08*
